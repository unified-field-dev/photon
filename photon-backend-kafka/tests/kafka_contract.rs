...
#[tokio::test]
#[ignore = "requires PHOTON_KAFKA_BROKERS and live Kafka"]
async fn kafka_append_subscribe_checkpoint_roundtrip() {
    if !kafka_available() {
        return;
    }

    let port = KafkaStoragePort::builder()
        .from_env_defaults()
...
#[tokio::test]
#[ignore = "requires PHOTON_KAFKA_BROKERS and live Kafka"]
async fn kafka_crypto_side_effect_matches_mem() {
    if !kafka_available() {
        return;
    }

    let _crypto = TransportCrypto::from_bytes(*b"photon-dev-transport-key-32bytes");
    let port = KafkaStoragePort::from_env().await.expect("connect kafka");
...
#[tokio::test]
#[ignore = "requires PHOTON_KAFKA_BROKERS and live Kafka"]
async fn kafka_dual_subscriber_fanout() {
    if !kafka_available() {
        return;
    }

    let port = Arc::new(KafkaStoragePort::from_env().await.expect("connect kafka"));
...
#[tokio::test]
#[ignore = "requires PHOTON_KAFKA_BROKERS and live Kafka"]
async fn kafka_tail_only_fanout() {
    if !kafka_available() {
        return;
    }
...
#[tokio::test]
#[ignore = "requires PHOTON_KAFKA_BROKERS and live Kafka"]
async fn kafka_keyed_shard_subscriptions_are_disjoint() {
    if !kafka_available() {
        return;
    }
...
#[tokio::test]
#[ignore = "requires PHOTON_KAFKA_BROKERS and live Kafka"]
async fn kafka_keyed_durable_consumer_reconnect() {
    if !kafka_available() {
        return;
    }
...
#[tokio::test]
#[ignore = "requires PHOTON_KAFKA_BROKERS and live Kafka"]
async fn kafka_keyed_durable_group_id_stable() {
    if !kafka_available() {
        return;
    }
...
#[tokio::test]
#[ignore = "requires PHOTON_KAFKA_BROKERS and live Kafka"]
async fn kafka_sharded_topic_append_subscribe_checkpoint() {
    if !kafka_available() {
        return;
    }

    std::env::set_var("PHOTON_KAFKA_TOPIC_SHARDS", "4");
    let port = Arc::new(
        KafkaStoragePort::builder()
            .from_env_defaults()
            .replay_cursor(ReplayCursor::StreamSeq)
            ...
            .build()
            .await
            .expect("connect sharded kafka"),
    );
    std::env::remove_var("PHOTON_KAFKA_TOPIC_SHARDS");
    ...
    let (shard, local) = photon_backend_kafka::stream_shard::decompose_seq(published.seq);
...
#[tokio::test]
#[ignore = "requires PHOTON_KAFKA_BROKERS and live Kafka"]
async fn kafka_sharded_unkeyed_fanin_subscribe() {
    if !kafka_available() {
        return;
    }
