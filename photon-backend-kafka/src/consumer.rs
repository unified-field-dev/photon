//! Kafka consumer setup for Photon subscriptions.
...
fn start_offset(replay_cursor: ReplayCursor, after_seq: Option<i64>, high_watermark: i64) -> i64 {
    match replay_cursor {
        ReplayCursor::TailOnly => high_watermark,
        ReplayCursor::StreamSeq => after_seq.filter(|&s| s >= 0).unwrap_or(high_watermark),
    }
}
...
async fn ensure_topic_cached(
    ...
    if ensured_topics.contains(kafka_topic) {
        return Ok(());
    }
    crate::topic::ensure_data_topic(client, config, kafka_topic).await?;
    ensured_topics.insert(kafka_topic.to_string());
    Ok(())
}
...
fn subscribe_single(
    ...
    let kafka_topic = kafka_topic_for(&config, topic_shard, &topic_name);
    ...
    Box::pin(stream! {
        if let Err(e) = ensure_topic_cached(&client, &config, &ensured_topics, &kafka_topic).await {
            yield Err(e);
            return;
        }

        let partition_client = match client
            .partition_client(kafka_topic.clone(), 0, UnknownTopicHandling::Retry)
            .await
        {
            Ok(client) => client,
            Err(e) => {
        ...
        let mut offset = start_offset(config.replay_cursor, effective_after, high_watermark);
        let topic = topic_name.clone();

        loop {
            let (records, new_high) = match partition_client
            ...
            for record_and_offset in records {
                ...
                match decode_record(&record_and_offset.record, record_and_offset.offset) {
                    Ok(mut decoded) => {
                        normalize_event_seq(
                            &mut decoded.event,
