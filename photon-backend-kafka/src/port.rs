//! Kafka [`StoragePort`] implementation.
...
use crate::config::{KafkaConfig, KafkaStoragePortBuilder, ReplayCursor, BROKERS_ENV};
use crate::connect::{connect_kafka, SharedClient};
use crate::consumer::subscribe_stream;
use crate::publish::PublishPipeline;
use crate::stream_shard::{composite_seq, kafka_topic_for, pick_shard, publish_routing_key};
...
/// # Errors
///
/// Returns an error when `PHOTON_KAFKA_BROKERS` is unset.
pub fn kafka_brokers_from_env() -> Result<String> {
    std::env::var(BROKERS_ENV).map_err(|_| {
        PhotonError::Internal(format!("{BROKERS_ENV} not set for kafka storage adapter"))
    })
}

/// Kafka-backed storage port.
pub struct KafkaStoragePort {
    client: SharedClient,
    config: KafkaConfig,
...
impl KafkaStoragePort {
    ...
    #[must_use]
    pub fn builder() -> KafkaStoragePortBuilder {
        KafkaStoragePortBuilder::new()
    }

    /// Connect using env (`PHOTON_KAFKA_*` defaults via builder).
    ///
    /// # Errors
    ///
    ...
    #[must_use]
    pub const fn config(&self) -> &KafkaConfig {
        &self.config
    }

    async fn connect_with_config(config: KafkaConfig) -> Result<Self> {
        warn_replication_settings(&config);
        let client = connect_kafka(&config).await?;
...
impl KafkaStoragePortBuilder {
    /// Connect and return a configured [`KafkaStoragePort`].
    ///
    /// # Errors
    ///
    /// Returns an error when configuration or connection fails.
    pub async fn build(self) -> Result<KafkaStoragePort> {
        let config = self.resolve()?;
        KafkaStoragePort::connect_with_config(config).await
    }
}

#[async_trait]
impl StoragePort for KafkaStoragePort {
    fn capabilities(&self) -> StorageCapabilities {
        StorageCapabilities::broker("kafka")
    }
