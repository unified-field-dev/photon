//! Builder and resolved configuration for the Fluvio storage adapter.
...
pub const ENDPOINT_ENV: &str = "PHOTON_FLUVIO_ENDPOINT";

/// Environment variable for topic name prefix.
pub const PREFIX_ENV: &str = "PHOTON_FLUVIO_TOPIC_PREFIX";
...
pub struct FluvioConfig {
    /// Fluvio Streaming Controller endpoint.
    pub endpoint: String,
    ...
    /// Independent Fluvio topics for ingress sharding (`1` = legacy single layout).
...
pub const REPLAY_CURSOR_ENV: &str = "PHOTON_FLUVIO_REPLAY_CURSOR";

/// Environment variable for synchronous publish ack (`1` / `0`).
pub const SYNC_ACK_ENV: &str = "PHOTON_FLUVIO_SYNC_ACK";

/// Environment variable for max in-flight publishes per port.
pub const MAX_INFLIGHT_ENV: &str = "PHOTON_FLUVIO_MAX_INFLIGHT";
...
/// ```
///
/// See also: [`FluvioConfig`] (resolved snapshot), crate-level topic mapping in [`crate`](index.html).
#[derive(Default)]
pub struct FluvioStoragePortBuilder {
...
impl FluvioStoragePortBuilder {
    ...
    /// Fill unset fields from `PHOTON_FLUVIO_*` environment variables.
    #[must_use]
    ...
    /// # Errors
    ///
    /// Returns an error when required fields (endpoint) are missing.
    pub fn resolve(self) -> Result<FluvioConfig> {
